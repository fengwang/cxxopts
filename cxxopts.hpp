...
namespace cxxopts
{
    ...
    inline std::string& stringAppend( std::string& s, const std::string& a )
    {
        return s.append( a );
    }

    inline std::string& stringAppend( std::string& s, size_t n, char c )
    {
        return s.append( n, c );
    }

    template <typename Iterator>
    std::string& stringAppend( std::string& s, Iterator begin, Iterator end )
    {
    ...
    struct option_syntax_exception : public OptionParseException
    {
        explicit option_syntax_exception( const std::string& text ) : OptionParseException( "Argument " + LQUOTE + text + RQUOTE + " starts with a - but has incorrect syntax" ) { }
    };

    struct option_not_exists_exception : public OptionParseException
    {
    ...
    struct missing_argument_exception : public OptionParseException
    {
    ...
    struct option_requires_argument_exception : public OptionParseException
    {
    ...
    struct option_not_has_argument_exception : public OptionParseException
    {
        option_not_has_argument_exception ( const std::string& option,
                                            const std::string& arg ) : OptionParseException( "Option " + LQUOTE + option + RQUOTE + " does not take an argument, but argument " + LQUOTE + arg + RQUOTE + " given" ) { }
    };

    struct option_not_present_exception : public OptionParseException
    {
    ...
    struct option_has_no_value_exception : public OptionException
    {
        explicit option_has_no_value_exception( const std::string& option ) : OptionException( option.empty() ?  ( "Option " + LQUOTE + option + RQUOTE + " has no value" ) : "Option has no value" ) { }
    };

    struct argument_incorrect_type : public OptionParseException
    {
        explicit argument_incorrect_type ( const std::string& arg ) : OptionParseException( "Argument " + LQUOTE + arg + RQUOTE + " failed to parse" ) { }
    };

    struct option_required_exception : public OptionParseException
    {
    ...
    {
        static_assert( std::is_base_of<std::exception, T>::value, "throw_or_mimic only works on std::exception and " "deriving structes" );
        throw T{text};
    }
    ...
    namespace values
    {
        ...
        template <typename T>
        struct abstract_value : public Value
        {
        ...
        template <typename T>
        struct standard_value : public abstract_value<T>
        {
        ...
        template <>
        struct standard_value<bool> : public abstract_value<bool>
        {
    ...
    {
            OptionDetails ( std::string short_, std::string long_, std::string desc, std::shared_ptr<const Value> val ) : m_short( std::move( short_ ) ), m_long( std::move( long_ ) ), m_desc( std::move( desc ) ),
    ...
    struct ParseResult
    {
            ...
            ParseResult( NameHashMap&& keys, ParsedHashMap&& values, std::vector<KeyValue> sequential, std::vector<std::string>&& unmatched_args ) : m_keys( std::move( keys ) ), m_values( std::move( values ) ),
                m_sequential( std::move( sequential ) ), m_unmatched( std::move( unmatched_args ) ) { }
    ...
    struct Options
    {
            ...
            void add_option ( const std::string& group, const std::string& s, const std::string& l, std::string desc, const std::shared_ptr<const Value>& value, std::string arg_help );
    ...
    namespace
    {
        ...
        std::string format_option ( const HelpOptionDetails& o )
        {
            const auto& s = o.s;
            const auto& l = o.l;
            std::string result = "  ";
        ...
        std::string format_description ( const HelpOptionDetails& o, size_t start, size_t width )
        {
            auto desc = o.desc;
    ...
    inline void OptionParser::parse_option ( const std::shared_ptr<OptionDetails>& value, const std::string&, const std::string& arg )
    {
    ...
    inline void Options::add_option ( const std::string& group, const std::string& s, const std::string& l, std::string desc, const std::shared_ptr<const Value>& value, std::string arg_help )
    {
    ...
    inline std::string Options::help_one_group( const std::string& g ) const
    {
    ...
    inline void Options::generate_group_help ( std::string& result, const std::vector<std::string>& print_groups ) const
    {
    ...
    inline void Options::generate_all_groups_help( std::string& result ) const
    {
